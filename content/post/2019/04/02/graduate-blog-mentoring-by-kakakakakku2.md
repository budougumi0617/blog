+++
title= "@kakakakakkuさんの２回目のブログメンティーを卒業した  #ブログメンタリング"
...
graduate-blog-mentoring-by-kakakakakku2


1月から3月まで[@kakakakakku](https://twitter.com/kakakakakku)さんのブログメンタリングを受けていた。
この記事はブログメンタリングの振り返りとなる。

# TL;DR
- 毎週書いているのにPVが伸びない = 記事の質がよくない？と思い二回目のブログメンタリングを受けた
...
まず、私は昨年の4月から6月に[@kakakakakku](https://twitter.com/kakakakakku)さんにブログメンターになっていただき、一度ブログメンタリングを受けていた。一度目のブログメンタリングを受けたあとの感想や結果報告はこちら。
...
ブログメンタリングを受けたあとも継続して週に1回ブログ記事を書いて完全にブログを習慣化をすることはできた。（月に一回の登壇などもしていた）。  
が、アクセス数が伸び悩んだりしていて、昨年末は以下のセルフ目標をクリアできず伸び悩んでいた。

- 月間1万PV（8,000PV~9,000PV程度で頭打ちしていた）
- はてブがつかない（最高で30はてブ弱）
...
- User Heat
  - https://userheat.com/
  - サイトが月間30万PVまでならば無料でヒートマップ分析をしてくれる。
...
Twitterのフォロワー数やPV、ブログの被はてブ数を週次でカウントしていた集計結果は以下の通り。昨年までは200程度だったブログ内はてブは3ヶ月間で1,300増加した。
...
はてブ上位の記事は以下の通り。念願のGo（技術記事）ではてブに載ることもできた（はてブ数は2019/04/02時点のもの）。
...
- ネガティブコメントを咀嚼するメンタル
- 単純にPVが多いとモチベーションに直結する。

また、2月には[人生2回目の20分枠での社外発表を行なった](https://gocon.connpass.com/event/118022/)が、参加者のみなさんから多数指摘やコメントをいただくことができた。（媒体は違うとはいえ、）普段からブログにアウトプットしてストーリーを考えたり、調査を続けていたからこそちゃんと資料が作れたかなと思う。
...
# 今後の課題
当初の目的は達成できたが、メンタリング終了時に以下の課題を[@kakakakakku](https://twitter.com/kakakakakku)さんに言い渡された。

- 普段の業務での貢献がブログにあまり出ていないこと
- 登壇実績などのコミュニティでの知名度とOSS実績の差
- 登壇など表に出る機会が多いわりにTwitterフォロワー数が少ないこと

どれも痛い指摘ばかりだった。たしかに普段の業務で業務中に取り扱っている話題や事例的な内容は少ない。うまく汎化できていなかったり、そもそも[社内のめちゃ強エンジニアのアウトプットにタダ乗り](https://developers.freee.co.jp/entry/service-infra-and-wire)して業務をしているというところもある。もっと実践導入力を上げていきたい。  

また、OSS活動などもその通りで、自分はもともと「あまり技術力がないので運営などでコミュニティにコミットしていこう」という感じだった。  
とはいえそれでもドキュメントに対するコミットなどは出来るはずで、あまり言い訳はできない。そもそも「なければ作る」ではなく「なければ諦める」になっている部分もあるので、自分や周りのNeedesを深掘りして自分にいま必要な作っていきたい。  
最近興味がある[analysisパッケージ](https://godoc.org/golang.org/x/tools/go/analysis)を使ったOSS開発などに取り組んでいく。
