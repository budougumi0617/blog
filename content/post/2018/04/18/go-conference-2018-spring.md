...
https://golang.org/src/runtime/type.go#L348
- たとえは`map[key]`で値を取得する実装はここ。
https://golang.org/src/runtime/hashmap.go#L343
- 実行時にアルゴリズムを渡せるので、C++のテンプレートのように型ごとのmapが作られること無くバイナリの誇大化を避けている。
...
自分も設計を考えるときはひとつのstructに入れてレシーバメソッド生やすような考え方をよくしてしまうので、とても参考になった。
...
htnl、cssやjs内のURLの置換もしてローカルでページを再現していたらしいので、
正規表現やフロントエンド言語の構造解析の勉強になりそう。

# go-selfupdate-github でツールを「自己アップデート」できるようにする
...
ただ、`go get`に関して言うとブランチかtagに`go1`がつけて公開しておくとHEADへの追従は起きないらしい。
...
# Replaced Backlog Git Server from Perl to Go
